...
def create_time_series_correlation_scatter_graphs(result_set, output_folder, timesteps=None, result_keys=None,
                                                  show=False):
    if not result_keys:
        result_keys = result_set.result_keys()
    if not timesteps:
        timesteps = result_set.timesteps()
    ...
    for parameter in uncertain_params:
        ...
        samples = [result_set.parameter_samples()[parameter][n] for n in indices]
        for t in timesteps:
            all_results = {r: [v[n] for n in indices] for r, v in result_set.result_data(timestep=t).iteritems()}
            for result_key in result_keys:
                result_values = all_results[result_key]
                plot_scatter_graph(samples, result_values, '', parameter, result_key, True,
                               output_folder + '/' + parameter + '_' + result_key + '_' + t, show)
